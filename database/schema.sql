...
-- Generated from frontend domain model (React mock data + page flows)
-- =============================================================================

SET NAMES utf8mb4;
SET FOREIGN_KEY_CHECKS = 0;

CREATE DATABASE IF NOT EXISTS techtrade_sa
...
CREATE TABLE roles (
  ...
  code          VARCHAR(32)  NOT NULL,  -- buyer, seller, admin, moderator
...
CREATE TABLE users (
  ...
  trust_score       DECIMAL(3,2) NOT NULL DEFAULT 0.00,  -- 0.00–5.00
  ...
  -- WhatsApp TTL Protocols page (user default for future trade bridges)
  whatsapp_opt_in           TINYINT(1) NOT NULL DEFAULT 0,
  whatsapp_ttl_preset       ENUM('24h','7d','90d','custom') NOT NULL DEFAULT '24h',
  whatsapp_ttl_custom_days  TINYINT UNSIGNED NULL,  -- required when preset = custom (1–365)
  whatsapp_ttl_updated_at   DATETIME NULL,
...
CREATE TABLE transactions (
  ...
  status              ENUM('pending','committed','completed','cancelled','disputed') NOT NULL DEFAULT 'pending',
...
CREATE TABLE whatsapp_bridges (
  ...
  CONSTRAINT chk_whatsapp_bridges_custom_days CHECK (
    ttl_preset <> 'custom' OR (ttl_custom_days IS NOT NULL AND ttl_custom_days BETWEEN 1 AND 365)
  )
) ENGINE=InnoDB;
...
CREATE TABLE user_reports (
  ...
  reason_code     ENUM('payment','communication','item_issue','no_show','other') NOT NULL,
  details         TEXT NOT NULL,
...
INSERT INTO categories (group_label, name, slug, spec_table, sort_order) VALUES
  ('COMPONENTS', 'GPUs',          'gpus',          'gpus',         1),
  ('COMPONENTS', 'CPUs',          'cpus',          'cpus',         2),
  ('COMPONENTS', 'Motherboards',  'motherboards',  'motherboards', 3),
  ('COMPONENTS', 'PSUs',          'psus',          'psus',         4),
  ('COMPONENTS', 'Memory',        'memory',        'memory',       5),
  ('COOLING',    'AIOs',          'aios',          'aios',        10),
  ('COOLING',    'Air Coolers',   'air-coolers',   'air_coolers', 11),
  ('COOLING',    'Case Fans',     'case-fans',     'case_fans',   12),
