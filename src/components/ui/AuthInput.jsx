...
const AuthInput = ({
	...
	const inputId =
		id || `cyber-input-${label?.replace(/\s+/g, "-").toLowerCase()}`;
	...
	return (
		<div className={`w-full space-y-2 ${className}`}>
			<label
				htmlFor={inputId}
				className="block text-slate-400 text-sm md:text-base font-black uppercase tracking-[0.3em] ml-1 font-mono"
			>
				{label}
			</label>
			...
			<div className="relative group">
				...
				<div
					className={`absolute top-2 right-2 w-1 h-1 pointer-events-none transition-colors duration-300
				...
				{isPassword && (
					<button
						...
						aria-label={
							showPassword ? "Mask credentials" : "Decrypt credentials"
						}
			...
			{error && typeof error === "string" && (
				<div
					id={errorId}
					role="alert"
					className="flex items-center gap-1.5 text-rose-500 text-sm md:text-base font-mono font-bold uppercase tracking-wider ml-1 mt-1.5 animate-in fade-in slide-in-from-top-1 duration-200"
				>
